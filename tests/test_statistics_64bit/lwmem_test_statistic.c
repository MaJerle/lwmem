...
test_run(void) {
     ...
     *
     * Test runs on 64-bit systems, where sizeof(void*) == 8 bytes
     */
    lwmem_get_stats(&stats);
    TEST_ASSERT(stats.mem_size_bytes == 112);
    TEST_ASSERT(stats.mem_available_bytes == 112);
    TEST_ASSERT(stats.minimum_ever_mem_available_bytes == 112);

    /* Allocate memory and continue */
    ptr = lwmem_malloc(64);
    lwmem_get_stats(&stats);
    TEST_ASSERT(stats.mem_size_bytes == 112);
    TEST_ASSERT(stats.mem_available_bytes == 32);
    TEST_ASSERT(stats.minimum_ever_mem_available_bytes == 32);

    /* Realloc and extend by 8 bytes */
    ptr = lwmem_realloc(ptr, 72);
    lwmem_get_stats(&stats);
    TEST_ASSERT(stats.mem_size_bytes == 112);
    TEST_ASSERT(stats.mem_available_bytes == 24);
    TEST_ASSERT(stats.minimum_ever_mem_available_bytes == 24);
