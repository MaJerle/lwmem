...
test_run(void) {
     ...
     */
    if (!lwmem_debug_create_regions(&regions_used, regions_count, 256)) {
    ...
    /* We know that sizeof internal metadata block is 16 bytes on 64-bit systems */
    printf("\r\n------------------------------------------------------------------------\r\n");
    printf("Allocating 4 pointers\r\n\r\n");
    ptr1 = lwmem_malloc(16);
    ptr2 = lwmem_malloc(8);
    ptr3 = lwmem_malloc(8);
    ptr4 = lwmem_malloc(32);
    lwmem_debug_print(1, 1);
    printf("\r\n------------------------------------------------------------------------\r\n");
